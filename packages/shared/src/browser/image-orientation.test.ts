...
describe("shouldForceManualOrientation", () => {
  it("forces manual orientation for EXIF-tagged JPEGs", () => {
    expect(shouldForceManualOrientation({ orientation: 6 })).toBe(true);
  });

  it("skips manual orientation when EXIF orientation is normal", () => {
    expect(shouldForceManualOrientation({ orientation: 1 })).toBe(false);
  });
});
