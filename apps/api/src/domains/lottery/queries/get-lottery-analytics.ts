...
import {
  GET_LOTTERY_ANALYTICS_SCHEMA,
  LOTTERY_OUTCOME,
} from "#domains/lottery/internal/lottery.constants.ts";
import { eq } from "drizzle-orm";

export const getLotteryAnalytics = publicProcedure
  .input(GET_LOTTERY_ANALYTICS_SCHEMA.optional())
  .query(async ({ input }) => {
    ...
    const events =
      sessionId != null
        ? await db
            ...
            .all()
        : await db.select().from(lotteryEventTable).all();
